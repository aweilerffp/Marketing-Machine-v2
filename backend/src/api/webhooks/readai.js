...
// Read.ai webhook handler with company authentication
router.post('/:companyId/:token', async (req, res) => {
  try {
    console.log('📥 Received Read.ai webhook:', req.body);

    const { companyId, token } = req.params;
    console.log(`🔍 DEBUG Webhook params - companyId: ${companyId || 'NULL'}, token: ${token ? token.substring(0, 10) + '...' : 'NULL'}`);
    ...
    if (companyId && token) {
      console.log(`🔍 DEBUG Looking up company with id: ${companyId}`);
      ...
      console.log(`🔍 DEBUG targetCompany.id: ${targetCompany.id}`);
    } else {
      console.log(`⚠️ DEBUG No companyId or token in URL params - webhook called without company context`);
    }
...
// Legacy Read.ai webhook handler (without authentication)
