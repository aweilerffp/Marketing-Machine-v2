...
// Test 1: New OpenAI API Key
const openaiKey = process.env.OPENAI_API_KEY;
if (openaiKey && openaiKey.startsWith('sk-proj-E12Dr')) {
  console.log('✅ 1. New OpenAI API key loaded');
  console.log('   Key prefix: ' + openaiKey.substring(0, 20) + '...');
  passed++;
} else {
  console.log('❌ 1. OpenAI key not updated');
  failed++;
}
...
if (anthropicKey && anthropicKey.startsWith('sk-ant-api03-DrSP')) {
  console.log('✅ 2. Anthropic API key rotated and loaded');
  console.log('   Key prefix: ' + anthropicKey.substring(0, 20) + '...');
  passed++;
} else {
  console.log('❌ 2. Anthropic API key not updated');
  failed++;
}
